...
findCUDAInstallPath verbosity platform = do
  result <- findFirstValidLocation verbosity platform (candidateCUDAInstallPaths verbosity)
  case result of
    Just installPath -> do
...
--
candidateCUDAInstallPaths :: Verbosity -> [(IO FilePath, String)]
candidateCUDAInstallPaths verbosity =
  [ (getEnv "CUDA_PATH",       "environment variable CUDA_PATH")
  , (findInPath,               "nvcc compiler executable in PATH")
  , (return "/usr/local/cuda", "default install location /usr/local/cuda")
  ]
  where
